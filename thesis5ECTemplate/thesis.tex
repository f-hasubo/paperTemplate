...
\eabstract{ % これ単体で複数ページにまたがる場合はエラーが出るので注意，アブスト内で改段落は禁止
  ...
  permutation problem.Thus, FDICA requires a permutation solver as a post-process. In recent years, independent
  ...
  the sources. On the other hand, it has been confirmed in the previous study that FDICA can separate very high
  quality for each frequency.Deep neural networks (DNNs) have been proposed to solve the permutation problem, however since the
  input frequency activation for each subband is estimated to be the same source or not, source separation of three or more sources 
  is not realistic from the viewpoint of computational complexity. 
}
...
\abstract{ % これ単体で複数ページにまたがる場合はエラーが出るので注意，アブスト内で改段落は禁止
 ...
 これまで，深層ニューラルネットワーク（deep neural networks: DNNs）を用いたパーミュテーション問題の解決法が提案されてきたが，サブバンド毎に入力された周波数アクティベーションが同一音源か否かを推定しているため，
 3音源以上の音源分離は計算量の観点から現実的ではないことが課題として挙げられる．
 本論文では，DNNに基づくパーミュテーション問題，またブロック単位でのパーミュテーション問題に対して新たなアルゴリズムを提案する．提案手法のDNNは，分離信号の複雑な時間周波数構造の特徴を事前に
 学習し，全周波数ビンについてパーミュテーション不整合が生じているか否かを予測する．このDNNの予測結果を用いて，パーミュテーション問題を解決する．
